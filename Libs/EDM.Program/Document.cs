...
namespace EDM.Program
{
    public class Document
    {
        ...
        #region --- Constructors ---
        public Document() { ProgramId = Setting.Session.ProgramId; ByUserId = EDM.Setting.Session.UserId; }
        public Document(String module) : this() { Module = module; }
        #endregion
        ...
        public DataSet GetBySMId()
        {
            try
            {
                ...
                String sql = MsSql.GetSqlStmt("p_GET_Documents", prms, out SqlforLog);
                Common.Log.Info(Module + ":EDM.Program.Document", "GetBySMId", SqlforLog);

                return MsSql.ExecuteQuery(sql);
            }
            catch (Exception ex) { Message = ex.Message; Common.Log.Error(Module, Module + ":EDM.Program.Document", "GetBySMId", ex); return null; }
        }
        ...
        public Boolean GetById()
        {
            try
            {
                ...
                String SqlforLog = string.Empty;
                String sql = MsSql.GetSqlStmt("p_GET_Document", prms, out SqlforLog);
                Common.Log.Info(Module + ":EDM.Program.Document", "GetById", SqlforLog);

                DataSet ds = MsSql.ExecuteQuery(sql);
                if (MsSql.IsEmpty(ds))
                {
                    Message = "Error fetching record.";
                    Common.Log.Info(Module + ":EDM.Program.Document", "GetById", SqlforLog + "|Error:" + Message);
                    return false;
                }
            ...
            catch (Exception ex) { Message = ex.Message; Common.Log.Error(Module, Module + ":EDM.Program.Document", "GetById", ex); return false; }
        }
        ...
        public Boolean Delete()
        {
            try
            {
                ...
                String SqlforLog = string.Empty;
                String sql = MsSql.GetSqlStmt("p_D_Document", prms, out SqlforLog);
                Common.Log.Info(Module + ":EDM.Program.Document", "Delete", SqlforLog);

                DataSet ds = MsSql.ExecuteQuery(sql);
                if (MsSql.IsEmpty(ds))
                {
                    Message = "Error deleting record.";
                    Common.Log.Info(Module + ":EDM.Program.Document", "Delete", SqlforLog + "|Error:" + Message);
                    return false;
                }
            ...
            {
                Message = ex.Message;
                Common.Log.Error(Module, Module + ":EDM.Program.Document", "Delete", ex);
                return false;
            }
        }
        ...
        public Boolean Add()
        {
            try
            {
                ...
                String SqlforLog = string.Empty;
                String sql = MsSql.GetSqlStmt("p_AU_Document", prms, out SqlforLog);
                Common.Log.Info(Module + ":EDM.Program.Document", "Add", SqlforLog);

                DataSet ds = MsSql.ExecuteQuery(sql);
                if (MsSql.IsEmpty(ds))
                {
                    Message = "Error adding record.";
                    Common.Log.Info(Module + ":EDM.Program.Document", "Add", SqlforLog + "|Error:" + Message);
                    return false;
                }
            ...
            {
                Message = ex.Message;
                Common.Log.Error(Module, Module + ":EDM.Program.Document", "Add", ex);
                return false;
            }
        }
        ...
        public Boolean Update()
        {
            try
            {
                ...
                String SqlforLog = string.Empty;
                String sql = MsSql.GetSqlStmt("p_AU_Document", prms, out SqlforLog);
                Common.Log.Info(Module + ":EDM.Program.Document", "Update", SqlforLog);

                DataSet ds = MsSql.ExecuteQuery(sql);
                if (MsSql.IsEmpty(ds))
                {
                    Message = "Error adding record.";
                    Common.Log.Info(Module + ":EDM.Program.Document", "Update", SqlforLog + "|Error:" + Message);
                    return false;
                }
            ...
            {
                Message = ex.Message;
                Common.Log.Error(Module, Module + ":EDM.Program.Document", "Update", ex);
                return false;
            }
        }
        #endregion
